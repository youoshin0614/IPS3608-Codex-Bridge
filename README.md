...
开关版复用完整自动化版的安全连接内核：每次首次连接或故障重连都必须先发送输出关闭并回读确认不高于 0.1 V。移植不包含电压、电流或原始串口接口；这些寄存器、设备方法、网络命令和 CLI 命令在本项目中均不存在。
...
- `health`：查看服务和串口状态；
- `status`：读取电压、电流和功率；为减少 CDC 通信量，温度轮询默认关闭；
- `on`：开启输出；
- `off`：关闭输出；
- `start` / `stop`：启动或安全停止本机服务；
- `ports`：列出串口；
- `diagnostics`：仅读取本地轮转日志，不接触串口。
...
`on`/`off` 会在内部等待并验证实际电压。默认采用按需遥测，不运行后台串口轮询；即使客户端每 0.5 秒执行一次 `status`，15 秒缓存窗口内也只返回缓存，不会向电源重复写读取命令。需要主动后台采样时可在 `start` 使用 `--poll-interval`，但频率越高越容易触发 IPS3608 USB CDC 故障。首次连接或重连后，物理设备至少稳定 3 秒才允许 `on`。`stop` 只有在回读确认输出不高于 0.1 V 后才会退出；若无法确认，服务继续运行并返回错误，必须重试 `off` 或人工关断。

Windows error 31/995、短写或写超时会把输出状态锁为 `unknown`，并阻止
`status` 自动重连和所有 `on` 请求。更换 USB 物理路径后仅执行 `off`；
只有实时回读不高于 0.1 V 才会解除互锁。可随时执行
`.\ips3608.cmd diagnostics` 查看结构化错误，不会访问设备。

执行 `off` 时不要并发执行 `status`：关断会优先独占串口完成恢复和回读，并发状态请求可能超时。2026-07-22 的带载验证记录见 [`docs/HARDWARE-TEST-2026-07-22.md`](docs/HARDWARE-TEST-2026-07-22.md)。
