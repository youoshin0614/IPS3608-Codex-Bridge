...
The service keeps a measurement monitor running at a configurable interval. This mirrors the continuous communication pattern of a desktop control application and avoids repeated USB CDC open/close cycles.

## Recovery

If telemetry fails, the service abandons the broken local serial handle and reconnects on the next monitor pass. A Windows CDC priming transition is attempted only during reconnect. The service never automatically retries an ambiguous output-on command. Output-off may be retried once because repeating it is safe.
